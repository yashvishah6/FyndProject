...
app.config['MYSQL_PASSWORD'] = 'dhairya123'
app.config['MYSQL_DB'] = 'fyndchat'
...
def not_logged_in(f):
    ...
    def wrap(*args, **kwargs):
        if 'logged_in' in session:
            flash('Unauthorized, You have logged in', 'danger')
...
class RegisterForm(Form):
    name = StringField('Name', [validators.DataRequired(), validators.length(min=3, max=50)], render_kw={'autofocus': True})
...
def register():
    ...
    if request.method == 'POST':
        ...
        if (result):
            ...
            flash('Password Requires:', 'danger')
            flash('1. uppercase letters: A-Z', 'danger')
            flash('2. lowercase letters: a-z', 'danger')
            flash('3. numbers: 0-9', 'danger')
            flash('4. any of the special characters: @#$%^&+=', 'danger')
